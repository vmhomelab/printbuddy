#!/usr/bin/env bash
#
# Printbuddy Native Installation Script
# Supports: Debian/Ubuntu, RHEL/Fedora/CentOS, Arch Linux, macOS
#
# Usage:
#   Interactive:  curl -fsSL https://raw.githubusercontent.com/vmhomelab/Printbuddy/main/install/install.sh -o install.sh && chmod +x install.sh && ./install.sh
#   Unattended:   ./install.sh --path /opt/printbuddy --port 8000 --yes
#
# Options:
...
show_help() {
    echo "Printbuddy Native Installation Script"
    echo ""
    ...
    echo "    ./install.sh --path /srv/printbuddy --port 3000 --tz America/New_York --yes"
    echo ""
...
download_printbuddy() {
    log_info "Downloading Printbuddy..."

    # Validate branch exists on remote before proceeding
    if ! git ls-remote --exit-code --heads https://github.com/vmhomelab/Printbuddy.git "$BRANCH" &>/dev/null; then
        log_error "Branch '$BRANCH' not found in the Printbuddy repository."
        log_info "Available branches:"
        git ls-remote --heads https://github.com/vmhomelab/Printbuddy.git | sed 's|.*refs/heads/|  - |'
        exit 1
    fi
    ...
    if [[ -d "$INSTALL_PATH/.git" ]]; then
        ...
        # user (not root, not printbuddy) unable to write .git into it.
        sudo mkdir -p "$INSTALL_PATH"
        sudo git clone --branch "$BRANCH" https://github.com/vmhomelab/Printbuddy.git "$INSTALL_PATH"
        sudo chown -R "$SERVICE_USER:$SERVICE_USER" "$INSTALL_PATH" 2>/dev/null || true
    fi

    log_success "Printbuddy downloaded to $INSTALL_PATH (branch: $BRANCH)"
}
...
EOF
    ...
    sudo mv /tmp/printbuddy.env "$env_file"
    if [[ "$OS_TYPE" != "macos" ]]; then
...
create_systemd_service() {
    ...
    cat > /tmp/printbuddy.service << EOF
[Unit]
...
EOF
    ...
    sudo mv /tmp/printbuddy.service /etc/systemd/system/printbuddy.service
    sudo systemctl daemon-reload

    log_success "Systemd service created"

    if prompt_yes_no "Enable Printbuddy to start on boot?" "y"; then
        sudo systemctl enable printbuddy
        log_success "Service enabled"
    fi

    if prompt_yes_no "Start Printbuddy now?" "y"; then
        sudo systemctl start printbuddy
        sleep 2
        if sudo systemctl is-active --quiet printbuddy; then
            log_success "Printbuddy is running"
        else
            log_warn "Service may have failed to start. Check: sudo journalctl -u printbuddy -f"
        fi
    fi
}
...
create_launchd_service() {
    ...
    local plist_path="$HOME/Library/LaunchAgents/com.printbuddy.app.plist"
...
<dict>
    <key>Label</key>
    <string>com.printbuddy.app</string>
    ...
    <string>$LOG_DIR/printbuddy.log</string>
    <key>StandardErrorPath</key>
    <string>$LOG_DIR/printbuddy.error.log</string>
</dict>
</plist>
EOF
    ...
    if prompt_yes_no "Load Printbuddy service now?" "y"; then
        launchctl load "$plist_path"
        sleep 2
        if launchctl list | grep -q "com.printbuddy.app"; then
            log_success "Printbuddy is running"
        else
            log_warn "Service may have failed to start. Check: cat $LOG_DIR/printbuddy.error.log"
        fi
    fi
}
...
main() {
    ...
    if [[ "$OS_TYPE" == "macos" ]]; then
        ...
        echo -e "    Start:   launchctl load ~/Library/LaunchAgents/com.printbuddy.app.plist"
        echo -e "    Stop:    launchctl unload ~/Library/LaunchAgents/com.printbuddy.app.plist"
        echo -e "    Logs:    tail -f $LOG_DIR/printbuddy.log"
    else
        echo -e "  ${BOLD}Manage service:${NC}"
        echo -e "    Status:  sudo systemctl status printbuddy"
        echo -e "    Start:   sudo systemctl start printbuddy"
        echo -e "    Stop:    sudo systemctl stop printbuddy"
        echo -e "    Logs:    sudo journalctl -u printbuddy -f"
    fi
    echo ""
    ...
    if [[ "$OS_TYPE" != "macos" ]]; then
        echo -e "    sudo systemctl restart printbuddy"
    fi
    echo ""
