export default {
  ...
  nav: {
    ...
    farmCommandCenter: 'ファームコマンドセンター',
    profiles: 'プロファイル',
    maintenance: 'メンテナンス',
    projects: 'プロジェクト',
    inventory: 'フィラメント',
  ...
  printers: {
    ...
    discovery: {
      ...
      scanSubnetMoonraker: 'サブネットをスキャンしてMoonrakerを検出',
      discoverNetwork: 'ネットワーク上のプリンターを検出',
      scanningSubnet: 'サブネットでBambuプリンターをスキャン中...',
      scanningSubnetMoonraker: 'サブネットでMoonrakerプリンターをスキャン中...',
      ...
      moonrakerSubnetNote: '下のポートでMoonraker/Klipper APIのJSONを探します（既定は7125、次にCosmOS/Fluidd用の80）。Bambuと同じサブネット一覧（DISCOVERY_EXTRA_SUBNETS含む）を使います。',
      moonrakerPorts: 'スキャンするポート',
      moonrakerPortsNote: 'カンマ区切り。CosmOS / プロキシ済みMoonrakerには80を指定。Moonraker APIのJSONを返すホストだけが一覧されます。',
  ...
  fileManager: {
    ...
    sourceDetails: 'ソース詳細',
    sourceArchive: 'アーカイブ済み MakerWorld 詳細',
    sourceArchiveLoadError: 'アーカイブ済みのソース詳細を読み込めませんでした。',
    sourceCapturedAt: '{{date}} にアーカイブ',
    gridView: 'グリッド表示',
    listView: 'リスト表示',
    lowDiskSpaceWarning: 'ディスク容量不足の警告',
  ...
  notifications: {
    ...
    selectedPrinters: '選択したプリンター',
    noPrintersAvailable: '利用可能なプリンターがありません',
    onlyFromSelectedPrinters: '選択したプリンターのイベントのみ通知を送信します',
    fromAllPrinters: 'すべてのプリンターのイベント通知を送信します',
    selectAtLeastOnePrinter: '少なくとも1台のプリンターを選択するか、すべてのプリンターを選択してください。',
  ...
  makerworld: {
    ...
    importDefaultsDescription: 'これらの選択を新しい MakerWorld インポートに自動的に適用します。インポートごとに上書きできます。',
    archiveDetailsByDefault: 'MakerWorld の詳細を既定でアーカイブ',
    archiveDetailsByDefaultDescription: 'モデルの説明、手順、表示画像をローカルに保存します。',
    useCoverAsThumbnailByDefault: 'アーカイブ済みカバーを既定でライブラリのサムネイルに使用',
    useCoverAsThumbnailByDefaultDescription: 'サムネイルがローカルにアーカイブしたカバーを使うため、詳細アーカイブも有効にします。',
    importDefaultsSaved: 'MakerWorld インポートの既定値を保存しました。',
    importDefaultsSaveFailed: 'MakerWorld インポートの既定値を保存できませんでした。',
    archiveDetailsDefaultControl: 'MakerWorld の詳細を既定でアーカイブ',
    coverThumbnailDefaultControl: 'アーカイブ済みカバーを既定でライブラリのサムネイルに使用',
